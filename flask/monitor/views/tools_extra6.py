...
# ====================================================================
_CASE_DB = Path(tempfile.gettempdir(), 'forensiclab_cases.db')
    ...
    if request.method == 'POST':
        ...
        if not f or not f.filename: error = '아티팩트 파일 필요'
            ...
            def _plaso_job(data, _job_id=None):
                ...
                _job_log(_job_id, 'log2timeline 시작', 15)
                tf = tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(f.filename)[1])
                tf.write(data); tf.close()
                plaso_out = tf.name + '.plaso'
                csv_out = tf.name + '.csv'
                try:
                    _job_log(_job_id, 'log2timeline.py 실행', 20)
                    r1 = subprocess.run(
                        ['log2timeline.py', '--storage_file', plaso_out, tf.name],
                        capture_output=True, text=True, timeout=1800)
                    _job_log(_job_id, f'log2timeline 종료 ({r1.returncode})', 60)
                    if r1.returncode != 0 and 'command not found' in (r1.stderr + '').lower():
                        return {'error': 'plaso 미설치 — pip install plaso',
                                'stderr': r1.stderr[-1000:]}
                    _job_log(_job_id, 'psort.py CSV 출력', 70)
                    r2 = subprocess.run(
                        ['psort.py', '-w', csv_out, plaso_out, '-o', 'l2tcsv'],
                        capture_output=True, text=True, timeout=600)
                    _job_log(_job_id, f'psort 종료', 90)
                    rows = []
                    ...
                    return {'rows': rows[:1000], 'total': len(rows),
                            'log2timeline_stdout': r1.stdout[-2000:],
                            'log2timeline_stderr': r1.stderr[-2000:]}
                except subprocess.TimeoutExpired:
                    return {'error': 'Plaso 시간 초과'}
                except FileNotFoundError:
                    return {'error': 'log2timeline.py 미설치 — pip install plaso (큰 패키지)'}
                except Exception as e:
                    return {'error': str(e)}
                finally:
                    for p in [tf.name, plaso_out, csv_out]:
                        try: os.unlink(p)
                        except Exception: pass
...
# 9. /tools/ocr-index — OCR 인덱싱 + 검색
# ====================================================================
_OCR_DB = Path(tempfile.gettempdir(), 'forensiclab_ocr_idx.db')
